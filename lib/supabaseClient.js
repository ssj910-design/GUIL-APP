...
// 로그인 토큰이 만료되면(7일, app/api/login/route.js) PostgREST가 401 + "exp" claim 관련 메시지(PGRST303 등)를
...
// 이걸로 1000행씩 끝까지 나눠 받는다. orderBy({column, ascending})를 주면 페이지마다 정렬을
// 걸어서 받는다 — range() 페이지네이션은 순서가 안정적이어야 페이지 간 누락·중복이 없다.
// filter: (q) => q 형태로 조건을 더 걸 때 (예: 취소된 고장 제외)
export async function fetchAll(table, select = "*", orderBy = null, filter = null) {
  const rows = [];
  const PAGE = 1000;
