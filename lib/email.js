...
function buildHtml(quote, hasCard, noticeMessage) {
  const bodyHtml = buildMessage(quote, noticeMessage, escapeHtml).replace(/\n/g, "<br>");
  const cardHtml = hasCard
    ? `<br><br><img src="cid:companycard" alt="명함" style="max-width:420px;width:100%;height:auto;display:block;" />`
    : "";
  return `<div style="font-family:'Malgun Gothic',sans-serif;font-size:14px;line-height:1.7;color:#222;">${bodyHtml}<br><br>${HTML_SIGNATURE}${cardHtml}</div>`;
}
...
export async function sendQuoteEmail({ to, cc, quote, pdfUrl, noticeMessage, attachmentUrls }) {
  ...
  const cardPath = path.join(process.cwd(), BRAND.assets.card);
  ...
  await transporter.sendMail({
    ...
    html: buildHtml(quote, hasCard, noticeMessage),
    attachments,
  });
}
