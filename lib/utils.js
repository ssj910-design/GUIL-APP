...
// 최근 30일간 현장별 고장 건수 — 집중관리현장 판단용. 처리완료 여부와 무관하게 전부 센다
// (기사가 처리완료해도 30일 내 누적 3회면 계속 집중관리 대상이어야 함).
// 기준은 호기 단위다 — 한 현장에 호기가 여러 대면 그걸 합산한 건수가 아니라,
// 그중 한 호기라도 30일 내 3회 이상 고장 났는지로 판단한다(호기별 최댓값을 반환).
export function failures30dBySite(failures) {
  const cutoff = Date.now() - 30 * 24 * 60 * 60 * 1000;
  const perUnit = new Map(); // `${siteId}|${elevatorNo}` -> count
  ...
  for (const [key, count] of perUnit) {
    const siteId = key.slice(0, key.indexOf("|"));
    bySite.set(siteId, Math.max(bySite.get(siteId) ?? 0, count));
  }
  return bySite;
}
