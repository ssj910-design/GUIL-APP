...
// 견적 총액(천단위 절사, VAT별도) — QuoteItemsModal·quotePdf와 같은 계산식.
// 알림톡 #{총금액} 등 "저장된 견적 행"에서 총액이 필요한 곳용 (items 키 이름만 흡수).
export function quoteGrandTotal(quote) {
  const items = quote.quoteItems ?? quote.items ?? [];
  const itemsSubtotal = items.reduce((s, it) => s + Number(it.qty || 0) * Number(it.unitPrice || 0), 0);
  const subtotal = itemsSubtotal + Number(quote.transportCost || 0) + Number(quote.safetyCost || 0) + Number(quote.profit || 0);
  return Math.floor(subtotal / 1000) * 1000;
}
