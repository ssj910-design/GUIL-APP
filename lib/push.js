...
/**
 * 안드로이드 알림 채널을 만들어둔다 — 채널은 한 번 만들어지면 중요도를 코드로 못 바꾸므로
 * (기기에 이미 만들어진 채널은 그대로 남는다), id/importance를 바꿀 땐 새 id로 채널을 새로 만들어야 한다.
 * 권한 여부와 무관하게 호출 가능해서(권한은 "알림이 뜨는지"만 좌우, 채널 생성 자체는 별개) 앱 시작 시점에도 부를 수 있다.
 */
export async function ensureNativeChannels() {
  ...
  // "높음" — 소리·진동은 있지만 헤드업 배너까진 필요 없는 알림용 (안드로이드 기본 채널과 같은 등급).
  await PushNotifications.createChannel({
    id: "high",
    name: "높음 알림",
    importance: 3, // DEFAULT — 소리·진동·알림함 표시는 되지만 헤드업은 안 뜬다
    visibility: 1,
    vibration: true,
  }).catch(() => {});
  // "긴급" — 다른 앱 사용 중에도 헤드업 배너로 떠야 하는 알림용. HIGH 채널이 따로 필요하다.
  await PushNotifications.createChannel({
    id: "urgent",
    name: "긴급 알림",
    importance: 4, // HIGH — 헤드업 배너로 뜬다
    visibility: 1,
    vibration: true,
  }).catch(() => {});
}
...
/**
 * 포그라운드에서 도착한 푸시를 OS 알림으로 다시 띄운다.
 *
 * 안드로이드는 앱을 보고 있는 동안 도착한 푸시를 화면에 안 띄운다(기본 정책 — 코드로 못 바꿈).
 * 그래서 앱이 직접 알림을 하나 만들어 같은 채널로 내보낸다 — 채널이 소리·진동·헤드업을 결정하므로
 * 긴급 건(urgent)은 앱을 켜둔 상태에서도 소리와 함께 뜬다. 알림함에 쌓이고 스와이프로 지워지는
 * 것도 OS가 알아서 해준다(앱이 그리던 배너는 이 셋 다 안 됐다).
 *
 * 채널 id는 서버가 data.channelId로 같이 보내준다(app/api/push/send) — 푸시 스키마에는 채널이
 * 안 실려 오기 때문이다.
 * @returns {Promise<boolean>} 실제로 알림을 띄웠으면 true (실패하면 호출부가 앱 안 배너로 대신한다)
 */
export async function showForegroundNotification(n) {
  ...
  try {
    ...
    await LocalNotifications.schedule({
      notifications: [{
        // id는 정수여야 한다 — 알림마다 달라야 앞 알림을 덮지 않고 쌓인다.
        id: Date.now() % 2147483647,
        channelId: n?.data?.channelId || "high",
        title: n?.title || "새 알림",
        body: n?.body || "",
        extra: { url: n?.data?.url || "/" },
        // 예약 없이 지금 바로 띄우는 알림이라 정확한 알람(AlarmManager)을 쓰지 않는데,
        // 이 옵션의 기본값이 true라 안드로이드 12+에서 "알람 및 리마인더" 권한이 없으면
        // 알림을 띄우기 전에 시스템 설정 화면부터 열어버린다 — 쓰지도 않을 권한이라 끈다.
        isExactNotification: false,
      }],
    });
    return true;
  } catch {
    return false; // 권한·플러그인 문제면 호출부가 앱 안 배너로 대신 알린다
  }
}

/**
 * 위에서 띄운 OS 알림을 탭했을 때 — 푸시 탭(onPushNotificationOpened)과 같은 방식으로 url을 넘긴다.
 * @returns {() => void} 리스너 해제 함수
 */
export function onLocalNotificationOpened(callback) {
  if (!Capacitor.isNativePlatform()) return () => {};
  let handle;
