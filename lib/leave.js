...
export function annualLeaveDays(hireDate, asOf) {
  ...
  if (years < 1) {
    // 1년 미만은 "개근한 달"마다 1일씩만 쌓인다 — 화면에서 asOf로 올해 12/31을 넘겨 보여줄 때가
    // 많은데, 그대로 쓰면 아직 지나지 않은 미래 달까지 채운 것처럼 부풀려진다. 오늘을 넘지 않게 막는다.
    const today = new Date().toISOString().slice(0, 10);
    const cappedAsOf = asOf < today ? asOf : today;
    return Math.max(0, Math.min(11, monthsOfService(hireDate, cappedAsOf)));
  }
