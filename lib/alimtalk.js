...
export async function sendQuoteAlimtalk({ to, quote, pdfUrl, noticeMessage }) {
  ...
  // #{link}에는 도메인 뒤 경로만 넣는다. 카카오 인앱 브라우저는 PDF를 못 열고 바로 다운로드로
  // 넘겨버려서, PDF 대신 같은 도메인의 Edge Function(supabase/functions/quote-viewer)을
  // 가리킨다 — 그 함수가 구글 뷰어로 PDF를 페이지 안에 그려서 보여준다.
