...
export async function sendQuoteAlimtalk({ to, quote, pdfUrl, noticeMessage }) {
  ...
  // 템플릿 버튼이 "https://kdptzotxnzpuwzdguzgh.supabase.co/#{link}" 형태(도메인 고정)라서,
  // #{link}에는 도메인 뒤 경로만 넣는다. 카카오 인앱 브라우저는 PDF도 못 열고(바로 다운로드로
  // 넘어감) 우리 도메인이 직접 내려주는 HTML도 못 그려서(원문을 텍스트로 그대로 보여줌 —
  // 헤더를 바꿔도 동일), 대신 같은 도메인의 Edge Function(supabase/functions/quote-viewer)이
  // 구글 뷰어(docs.google.com)로 302 리다이렉트시킨다 — 실제로 열리는 페이지는 우리 도메인이
  // 아니라 구글이라 카카오도 정상적인 웹사이트로 인식해 그대로 열어준다.
  const origin = new URL(pdfUrl).origin; // 템플릿 버튼과 같은 도메인이어야 한다(위 주석 참고)
  const link = `functions/v1/quote-viewer?url=${encodeURIComponent(pdfUrl)}`;
