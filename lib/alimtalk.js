...
export async function sendQuoteAlimtalk({ to, quote, pdfUrl, noticeMessage }) {
  ...
  //
  // PDF 대신 우리 도메인의 뷰어 페이지·구글 뷰어 리다이렉트를 다 시도해봤지만(원문을 텍스트로
  // 그대로 보여주거나 "페이지를 찾을 수 없음"), 카카오 인앱브라우저(알림톡)가 등록된 도메인
  // 외의 자유 탐색을 막는 것으로 보여 — 서버 응답을 바꿔도 소용없다. PDF 직접 링크가 유일하게
  // 실제로 동작하는 방식이라(눌러서 다운로드는 받아짐) 이걸로 둔다.
