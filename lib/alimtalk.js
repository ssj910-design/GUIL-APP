...
export async function sendQuoteAlimtalk({ to, quote, pdfUrl, noticeMessage }) {
  ...
  // #{link}에는 도메인 뒤 경로만 넣는다.
  //
  // 뷰어 페이지(Edge Function, supabase/functions/quote-viewer)를 거치게 해봤지만, 카카오
  // 인앱 브라우저가 실제 웹페이지로 열지 않고 응답 원문을 그대로 텍스트로 보여줘서
  // (헤더를 뭘로 바꿔도 동일 — 카카오 쪽에서 링크를 서버측으로 미리 가져와 텍스트만
  // 보여주는 것으로 추정) 오히려 PDF 직접 링크보다 더 못 쓰게 나왔다. PDF 직접 링크로
  // 되돌린다 — 최소한 "다운로드" 눌러서 파일은 받을 수 있다.
  const u = new URL(pdfUrl);
  const link = u.pathname.replace(/^\//, "") + u.search;
  const linkUrl = u.href;
