  ...
  const message = {
    ...
    kakaoOptions: {
      ...
      variables: {
        ...
        "#{총금액}": formatTotal(quote),
        "#{특이사항}": noticeContent(noticeMessage),
        // ⚠️ 승인된 템플릿(2026-08-07)의 버튼 링크가 도메인 고정이라 #{link} 변수가 없다 —
        // 버튼은 supabase 루트로만 간다. 실제 PDF로 보내려면 템플릿 버튼 링크를
        // "https://kdptzotxnzpuwzdguzgh.supabase.co/#{link}"로 바꿔 재승인받아야 한다.
        // 그전까지 PDF 링크는 SMS 대체발송 문구(buildFallbackText)에만 들어간다.
      },
