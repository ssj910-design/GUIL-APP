...
function formatTotal(quote) {
  ...
  if (!Number.isFinite(n) || n <= 0) return "견적서 참조";
  const total = quoteDisplayTotal(n, quote.vatIncluded);
  return `${total.toLocaleString()}원 (${quote.vatIncluded ? "VAT포함" : "VAT별도"})`;
}
...
//
// ⚠️ 이 API는 messageId(단수 콤마조인)도, messageIds(복수 반복)도 필터로 안 먹는다(실측
// 확인 — 공식 SDK가 요청 스키마에 선언은 해뒀지만 서버가 실제로 그 필터를 안 받아준다).
// 필터 없이 최신순으로 훑으면 정상적으로 결과가 나오길래, 그 방식으로 우회한다: 최신순
// 페이지를 넘겨가며(nextKey) 우리가 찾는 messageId가 나올 때마다 채우고, 다 채워지거나
// 페이지 한도에 닿으면 멈춘다. pending 건은 최근 것부터 우선이라 대부분 앞쪽 페이지에서
// 끝난다.
//
// statusCode 4000 = 수신 완료. 그 외 코드가 있으면 실패, 아직 없으면 진행 중.
const PAGE_LIMIT = 100;
const MAX_PAGES = 20; // 100개씩 최대 2,000건 — 2주치 pending 정도는 이 안에서 다 잡힌다.

export async function fetchAlimtalkStatus(messageIds) {
  const pending = new Set((messageIds ?? []).filter(Boolean));
  ...
  for (let page = 0; page < MAX_PAGES && pending.size > 0; page++) {
    ...
    if (!json?.nextKey) break;
    startKey = json.nextKey;
  }
  return out;
}
