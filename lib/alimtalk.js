...
// 발송 결과 조회 — messageId로 지금 상태를 직접 물어본다.
//
// 웹훅(/api/solapi-webhook)이 있는데도 이게 필요한 이유: 웹훅은 **솔라피 콘솔 설정과
// SOLAPI_WEBHOOK_TOKEN에 의존**한다. 둘 중 하나라도 어긋나면 결과가 영영 안 오고
// 화면에는 "확인 중"만 남는다(실제로 그 상태였다). 조회는 API 키만 있으면 되니
// 발송이 되는 환경이면 조회도 반드시 된다 — 웹훅은 빠른 길, 조회는 확실한 길이다.
//
// statusCode 4000 = 수신 완료. 그 외 코드가 있으면 실패, 아직 없으면 진행 중.
export async function fetchAlimtalkStatus(messageIds, opts = {}) {
  const ids = [...new Set((messageIds ?? []).filter(Boolean))];
  if (!ids.length) return {};

  const out = {};
  const rawResponses = [];
  // 이 API는 messageId(단수)에 콤마로 이어붙인 값을 줘도 필터가 안 걸린다 — 실제로는
  // messageIds(복수)를 같은 키로 여러 번 반복해서 줘야 한다(공식 SDK 소스 확인:
  // qs 직렬화 옵션 indices:false → messageIds=A&messageIds=B&... 형태). 기본 페이지
  // 크기(20)에 맞춰 20개씩 끊는다 — limit을 따로 안 줘도 한 번에 다 받힌다.
  for (let i = 0; i < ids.length; i += 20) {
    const qs = new URLSearchParams();
    for (const id of ids.slice(i, i + 20)) qs.append("messageIds", id);
    const res = await fetch(`${SOLAPI_LIST_ENDPOINT}?${qs}`, {
      headers: { Authorization: authHeader() },
    });
    if (!res.ok) {
      console.error("[alimtalk] 상태 조회 실패:", res.status, await res.text().catch(() => ""));
      if (opts.debug) rawResponses.push({ ok: false, status: res.status });
      continue;   // 일부 실패해도 나머지는 채운다
    }
    const json = await res.json().catch(() => ({}));
    if (opts.debug) rawResponses.push({ ok: true, status: res.status, json });
    for (const [id, m] of Object.entries(json?.messageList ?? {})) {
      const code = String(m.statusCode ?? "");
      out[id] = {
        status: code === "4000" ? "delivered" : code ? "failed" : "pending",
        statusCode: m.statusCode ?? null,
        statusMessage: m.reason ?? m.statusMessage ?? null,
      };
    }
  }
  if (opts.debug) out._raw = rawResponses;
  return out;
}

// TEMP DEBUG — 아무 필터 없이 계정에 메시지가 아예 조회되는지만 확인.
export async function debugListNoFilter() {
  const res = await fetch(`${SOLAPI_LIST_ENDPOINT}?limit=5`, { headers: { Authorization: authHeader() } });
  const json = await res.json().catch(() => null);
