...
// savePartLeafPhotos: 한 번에 여러 장(2장 이상)이 새로 추가되면 전부 onAdd 호출 —
// FileCarousel이 업로드 배치가 다 끝난 뒤 한 번만 onSave를 부르므로, 여기서는
// "이전 urls 대비 nextUrls에 몇 개가 새로 생겼는지"를 전부(하나만이 아니라) 판별해야 한다.
{
  const added = [];
  await savePartLeafPhotos({
    unitId: "unit1", category: "카도어", subcategory: null, part: "카도어 벤",
    mine, urls, nextUrls: ["u1", "u2", "new1", "new2"],
    onAdd: async (payload) => { added.push(payload.url); },
    onRemove: async () => { throw new Error("onRemove는 호출되면 안 됨"); },
  });
  assert.deepEqual(added, ["new1", "new2"]);
}
