...
export async function buildReplacementCertificatePdfBytes(cert) {
  ...
  function text(str, x, yPos, size, opts = {}) {
    ...
    // Bold 폰트는 안 쓰므로(위 주석 — 임베드가 몇 초씩 걸림) 꼭 굵어야 하는 곳(품목명)만
    // 같은 글자를 살짝 옆으로 겹쳐 찍어 굵게 보이게 한다.
  ...
  // 품목별 금액은 없어도(합계금액만 입력한 경우 등) 단가·금액 칸 자체는 항상 보여주고
  // 값이 없는 품목만 "-"로 비워둔다 — 예전엔 하나라도 없으면 칸 전체를 숨겼는데, 그러면
  // 합계금액만 입력한 건도 품목별 내역이 아예 안 보여 헷갈렸다.
  const COLS = [
  ...
  y -= 2;
  const totalLabel = "합계";
  // "견적서 참조"는 무상이 아니라 금액이 별도 견적서에 있다는 뜻이다 — is_free로 저장되지만
  // 고객 문서에 "무상"이라고 찍으면 안 된다(관리자웹 목록도 사유를 그대로 보여준다).
  const totalValue = cert.isFree
    ? (cert.freeReason === "견적서 참조" ? "견적서 참조" : "무상")
    : cert.totalCost != null ? fmtWon(cert.totalCost) : "-";
  ...
  // 한 줄에 3장이 여백 없이 꽉 차도록 타일 크기를 폭에서 역산한다(약 6cm). 예전 2장(약 9cm)은
  // 한 장에 2줄밖에 안 들어가 페이지가 많고 줄이 넘어갈 때 빈 공간이 컸다.
  const tileGap = 8;
  const cols = 3;
  const tileSize = (CONTENT_W - tileGap * (cols - 1)) / cols;
  const captionH = 16; // "교체 전 (N장)" 캡션 줄 높이
  ...
  // 페이지는 사진 "줄" 단위로 넘긴다 — 예전엔 묶음 전체 높이로 확인해서, 남은 공간에 안 들어가면
  // 묶음이 통째로 다음 장으로 밀려 앞 장 품목명 아래가 비었고, 한 페이지보다 긴 묶음(7장 이상)은
  // 아래가 잘렸다. 캡션은 첫 줄과 같은 장에 오도록 같이 확인한다.
  ...
  // 교체 전·후가 딱 1장씩이면 굳이 줄을 나눌 필요 없이 한 줄에 나란히 배치한다 — 여러 장이면
  // (drawPhotoStrip로) 각각 줄바꿈해서 보여준다.
  ...
  items.forEach((it, i) => {
    // 품목명이 혼자 앞 장 끝에 남지 않게 품목명 + 캡션 + 사진 첫 줄이 들어갈 공간을 같이 확인한다.
    ensureSpace(26 + captionH + tileSize + 10);
    text(`${it.name}${it.qty ? ` ${it.qty}` : ""}`, MARGIN_X, y - 12, 13, { heavy: true });
    y -= 26;
    ...
    if (beforeImgs.length === 1 && afterImgs.length === 1) {
      ...
      drawPhotoStrip(beforeImgs, "교체 전");
      drawPhotoStrip(afterImgs, "교체 후");
    }
    y -= 6;
  });
