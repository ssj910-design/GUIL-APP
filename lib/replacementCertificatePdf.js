...
// 폰트 파일(약 2MB×2)을 요청마다 디스크에서 다시 읽지 않도록 모듈 스코프에 캐싱해둔다 —
// 서버리스 함수가 warm 상태로 재사용될 때는 이 파일을 다시 읽지 않는다. 폰트 자체를
// PDF에 임베드(pdf-lib+fontkit 파싱)하는 비용은 여전히 매 요청 발생하지만(교체확인서는
// billings.certificate_pdf_url에 캐싱해 재생성 자체를 건너뛰는 방식으로 대응한다),
// 최소한 디스크 I/O는 줄인다.
...
export async function buildReplacementCertificatePdfBytes(cert) {
  ...
  cachedFontBold ??= fontBytes("Bold");
  const font = await pdfDoc.embedFont(cachedFontRegular);
  const fontBold = await pdfDoc.embedFont(cachedFontBold);
