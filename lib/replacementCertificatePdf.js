...
export async function buildReplacementCertificatePdfBytes(cert) {
  ...
  const font = await pdfDoc.embedFont(fs.readFileSync(path.join(process.cwd(), "public/fonts/NanumGothic-Regular.ttf")));
  const fontBold = await pdfDoc.embedFont(fs.readFileSync(path.join(process.cwd(), "public/fonts/NanumGothic-Bold.ttf")));
  ...
  // 부품별 사진(최대 8장)·서명 이미지를 미리 전부 내려받아 임베드해둔다 — 레이아웃을
  // 그리는 도중에 await가 섞이면 페이지 커서 계산이 꼬이기 쉬워, 그리기 전에 끝내둔다.
  const items = cert.items ?? [];
  const itemPhotos = await Promise.all(
    items.map(async (it) => ({
      before: await embedPhoto(pdfDoc, it.beforeUrl),
      after: await embedPhoto(pdfDoc, it.afterUrl),
    }))
  );
  ...
  if (iconImage) page.drawImage(iconImage, { x: MARGIN_X, y: y - iconH, width: iconW, height: iconH });
  const nameSize = 14.5;
  const nameAscent = fontBold.heightAtSize(nameSize, { descender: false });
  text(BRAND.name, MARGIN_X + iconW + 9, y - iconH / 2 - nameAscent / 2, nameSize, { bold: true });

  const titleSize = 20;
  const titleW = fontBold.widthOfTextAtSize("부품 교체 확인서", titleSize);
  text("부품 교체 확인서", MARGIN_X + CONTENT_W - titleW, y - 15, titleSize, { bold: true });
  const metaSize = 8.25;
  const docMetaText = `문서번호 ${cert.docNumber}   발급일자 ${cert.issuedDate}`;
  const docMetaW = font.widthOfTextAtSize(docMetaText, metaSize);
  text(docMetaText, MARGIN_X + CONTENT_W - docMetaW, y - 15 - 15, metaSize, { color: MUTED });
  ...
  items.forEach((it, i) => {
    const blockH = 15 + photoSize + 14;
    ensureSpace(blockH);
    text(`${it.name}${it.qty ? ` ${it.qty}` : ""}`, MARGIN_X, y - 9, 9.75, { bold: true });
    y -= 18;
    [["교체 전", itemPhotos[i]?.before], ["교체 후", itemPhotos[i]?.after]].forEach(([caption, img], slot) => {
      const px = MARGIN_X + slot * (photoSize + photoGap);
      rect(px, y, photoSize, photoSize, { borderColor: LINE_STRONG, borderWidth: 0.75 });
      if (img) {
        const scale = Math.min(photoSize / img.width, photoSize / img.height);
        const dw = img.width * scale, dh = img.height * scale;
        page.drawImage(img, { x: px + (photoSize - dw) / 2, y: y - photoSize + (photoSize - dh) / 2, width: dw, height: dh });
      } else {
        const msg = "사진을 불러올 수 없습니다";
        const w = font.widthOfTextAtSize(msg, 7.5);
        text(msg, px + (photoSize - w) / 2, y - photoSize / 2, 7.5, { color: MUTED });
      }
      const capW = font.widthOfTextAtSize(caption, captionSize);
      text(caption, px + (photoSize - capW) / 2, y - photoSize - 12, captionSize, { bold: true, color: INK_SOFT });
    });
    y -= photoSize + 24;
  });
  ...
  if (cert.approval?.method) {
    ...
    if (cert.approval.method === "서명") {
      ...
      text("서명자 연락처", infoX, y - 40, factLabelSize, { color: MUTED, bold: true });
      text(cert.approval.approverPhone || "-", infoX, y - 52, factValueSize, { bold: true });
      y -= sigH + 24;
    } else {
