...
export async function buildReplacementCertificatePdfBytes(cert) {
  ...
  // 한 줄에 2장이 여백 없이 꽉 차도록 타일 크기를 폭에서 역산한다.
  const tileGap = 8;
  const tileSize = (CONTENT_W - tileGap) / 2;
  ...
  function drawPhotoStrip(images, label) {
    ...
    y -= 16;
    if (count === 0) {
      drawTile(MARGIN_X, y, null);
      y -= tileSize + 10;
      return;
    }
    images.forEach((img, idx) => {
      const col = idx % cols, row = Math.floor(idx / cols);
      drawTile(MARGIN_X + col * (tileSize + tileGap), y - row * (tileSize + tileGap), img);
    });
