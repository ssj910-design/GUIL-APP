...
export const NOTIFICATIONS = [
  ...
  { key: "failure_cancelled", label: "고장 접수 취소됨", audience: "admin", level: "urgent", trigger: "instant", group: "고장",
    desc: "관리자 전원 + 배정돼 있던 기사에게 갑니다" },
  ...
  { key: "dispatch_no_response", label: "배정된 기사가 출동응답 안 함", audience: "engineer_admin", level: "urgent", trigger: "scheduled", built: true, group: "고장",
    defaultAudienceTiers: ["super", "manager"], // 자재담당관리자는 고장 대응에 관여하지 않아 기본 제외
    desc: "배정 후 배정 기사(출근중일 때만)에게는 3분마다, 관리자에게는 15분마다 (pg_cron이 매분 확인)" },
  { key: "failure_result_overdue", label: "출동 후 처리결과 5시간 미입력", audience: "engineer_admin", level: "high", trigger: "scheduled", built: true, group: "고장",
    defaultAudienceTiers: ["super", "manager"], // 자재담당관리자는 고장 대응에 관여하지 않아 기본 제외
    desc: "출동 후 5시간이 지나도 처리결과가 안 들어오면 배정 기사·관리자에게 딱 한 번 동시 발송 (pg_cron이 매분 확인)" },
  ...
  { key: "attendance_missing", label: "출근 체크 안 함", audience: "engineer", level: "low", trigger: "scheduled", built: true, group: "근무",
    desc: "09:01~10:00 매분(일반) · 오전반차는 12:01~13:00 매분, 체크 전까지 (연차·병가·공가·평일·공휴일·근로자의날 제외)" },
  { key: "attendance_report", label: "출근 미체크 인원 요약", audience: "admin", level: "low", trigger: "scheduled", built: true, group: "근무",
    desc: "09:10 기준 (연차·반차(오전)·병가·공가·평일·공휴일·근로자의날 제외)" },
  { key: "attendance_checkout_missing", label: "퇴근 체크 안 함", audience: "engineer", level: "low", trigger: "scheduled", built: true, group: "근무",
    desc: "18:00 1회(일반, 오전반차 포함) · 오후반차는 12:00 1회 (숙직·당직·연차·병가·공가·평일·공휴일·근로자의날 제외)" },
  { key: "attendance_checkout_report", label: "퇴근 미체크 인원 요약", audience: "admin", level: "low", trigger: "scheduled", built: true, group: "근무",
    desc: "18:30 1회 (숙직·당직·연차·오후반차·병가·공가·평일·공휴일·근로자의날 제외, 오전반차는 포함)" },
  ...
  { key: "quote_requested", label: "견적 신청 들어옴", audience: "admin", level: "normal", trigger: "instant", group: "자재",
    defaultAudienceTiers: ["super", "manager"], // 자재담당관리자는 견적 업무에 관여하지 않아 기본 제외
    desc: "자재담당관리자 제외" },
  ...
  { key: "quote_rejected", label: "내 견적 요청 반려·취소됨", audience: "engineer", level: "normal", trigger: "instant", group: "자재",
    desc: "신청 기사 + (지급완료 건이면) 할일 담당 기사" },
  { key: "material_rejected", label: "내 자재 신청 반려됨", audience: "engineer", level: "normal", trigger: "instant", group: "자재",
    desc: "관리자가 지급 전 신청을 반려했을 때, 신청 기사에게" },
