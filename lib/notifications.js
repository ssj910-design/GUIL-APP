...
export const NOTIFICATIONS = [
  ...
  { key: "failure_stale", label: "15분째 아무도 안 잡은 미배정 고장", audience: "admin", level: "urgent", trigger: "scheduled", built: true, group: "고장",
    desc: "미배정 15분 경과 시 관리자에게 (pg_cron이 매분 확인)" },
  { key: "dispatch_no_response", label: "배정된 기사가 출동응답 안 함", audience: "all", level: "urgent", trigger: "scheduled", built: true, group: "고장",
    desc: "배정 후 5분마다 배정 기사·관리자에게 (pg_cron이 매분 확인)" },
  ...
  { key: "supply_request_cancelled", label: "자재·견적 신청 취소됨", audience: "admin", level: "normal", trigger: "instant", group: "자재" },
...
/** 알림 설정 화면의 "받는 사람" 드롭다운 — audience가 "all"(전원)인 알림은 대상 아님(고정 전원) */
export const RECIPIENTS = {
  engineer: "기사",
  admin: "전체관리자",
  admin_super: "최고관리자",
  admin_manager: "중간관리자",
  admin_material: "자재담당관리자",
};

/**
 * 지금 이 알림이 실제로 누구에게 가는지 — 회사가 "받는 사람"을 바꿔뒀으면(audienceOverride) 그 값,
 * 아니면 카탈로그 기본(item.audience)을 RECIPIENTS 키 중 하나로 환산한다.
 */
export function recipientOf(item, orgSettings = {}) {
  const s = orgSettings[item.key] ?? {};
  const audience = s.audienceOverride || item.audience;
  if (audience === "engineer") return "engineer";
  if (audience === "admin") return s.audienceTier ? `admin_${s.audienceTier}` : "admin";
  return null; // "all"(전원)
}

/** RECIPIENTS 드롭다운에서 고른 값을 notify_settings에 저장할 {audienceOverride, audienceTier}로 되돌린다 */
export function recipientToPatch(value) {
  if (value === "engineer" || value === "admin") return { audienceOverride: value, audienceTier: null };
  return { audienceOverride: "admin", audienceTier: value.slice("admin_".length) };
}
