...
export const NOTIFICATIONS = [
  ...
  { key: "attendance_missing", label: "출근 체크 안 함", audience: "engineer", level: "low", trigger: "scheduled", built: true, group: "근무",
    desc: "09:01~09:30 매분, 체크 전까지 (연차·반차(오전)·병가·공가 제외)" },
...
/**
 * 관리자 알림(audience: "admin")만 받는 사람을 등급으로 좁힐 수 있다 — 여러 등급을 동시에 고를 수
 * 있어(예: 최고+중간만) 항상 배열로 돌려준다. 빈 배열(또는 저장된 값이 없음) = 전체 관리자.
 */
export function audienceTiersOf(item, orgSettings = {}) {
  return item.audience === "admin" ? (orgSettings[item.key]?.audienceTiers ?? []) : [];
}
