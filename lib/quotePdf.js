...
// 글씨가 전체적으로 작다는 요청(2026-08-07)으로 모든 폰트 크기·줄간격에 곱한다.
// 페이지 크기·여백·표 칸 너비는 그대로 두고(칸 안 글자는 truncateToWidth/wrapText가
// 폭에 맞춰 알아서 줄이거나 줄바꿈한다) 세로 방향 크기만 키운다.
const SCALE = 1.3;
const S = (n) => n * SCALE;
...
// 그때 wrapText 로 업그레이드.
const COLS = [
  { key: "name", label: "품명", width: 140, align: "center" },
  { key: "unitNo", label: "호기", width: 28, align: "center" },
  { key: "spec", label: "규격", width: 130, align: "center" },
  { key: "unit", label: "단위", width: 30, align: "center" },
  { key: "qty", label: "수량", width: 37, align: "center" },
  { key: "unitPrice", label: "단가", width: 75, align: "center" },
  { key: "amount", label: "금액", width: 75, align: "center" },
];
const ROW_H = S(18);
const HEADER_H = S(20);

// 자재비/인건비 내역 아래 공백 — 한 줄만.
const CATEGORY_PAD = ROW_H;
...
export async function buildQuotePdfBytes(quote) {
  ...
  const headerAscent = fontBold.heightAtSize(S(9), { descender: false });
  const totalAscent = fontBold.heightAtSize(S(9), { descender: false });
  ...
  function renderLayout() {
    ...
    if (logoPage) {
      ...
      const titleAscent20 = fontBold.heightAtSize(S(20), { descender: false });
      page.drawText("GUIL ELEVATOR", { x: MARGIN_X, y: logoCenterY - titleAscent20 / 2, size: S(20), font: fontBold, color: ACCENT });
    }
    ...
    const infoValueW = rightX - leftX - 10; // 견적명처럼 긴 값이 우측 공급자 정보란을 침범하지 않도록 제한
    const infoStartY = y;
    // 수신자/견적명은 값이 길 수 있어 라벨 바로 밑 줄에 값을 쓴다. 나머지는 라벨 옆에 값.
    function infoRowInline(label, value) {
      text(label, leftX, S(9), { bold: true });
      text(truncateToWidth(font, S(9), value, infoValueW - 55), leftX + 55, S(9));
      y -= S(14);
    }
    function infoRowStacked(label, value) {
      text(label, leftX, S(9), { bold: true });
      y -= S(14);
      text(truncateToWidth(font, S(9), value, infoValueW), leftX, S(9));
      y -= S(14);
    }
    infoRowInline("No.", quote.quoteNumber || "-");
    infoRowStacked("수신자", quote.recipientName || "귀중");
    infoRowStacked("견적명", quote.quoteTitle || "");
    infoRowInline("견적일", quote.quoteDate || "");
    infoRowInline("유효기간", "견적일로부터 1개월");
    const leftFinalY = y;
    ...
    [
      ...
      text(truncateToWidth(font, S(8), line2, rightW), rightX, S(8));
      y -= S(13);
    });
    y = infoStartY - 5 * S(14) - S(10);
    const ceoText = COMPANY.ceo + " (Confirmed and Signed by)";
    text(ceoText, rightX, S(8));
    let rightFinalY = y;
    if (sealImage) {
      const sealSize = 40;
      const sealY = y - sealSize / 2 + 5;
      const ceoTextW = font.widthOfTextAtSize(ceoText, S(8));
      ...
      rightFinalY = sealY; // 인감 하단이 서명 줄보다 아래로 내려온다
    }
    // 유효기간(왼쪽)과 "일금" 사이 공백 없이 바로 이어 붙인다 — 단, 오른쪽 인감 도장과는 겹치지 않게
    // 양쪽 중 더 아래(=더 작은 y)까지 내려간 쪽을 기준으로 삼는다.
    y = Math.min(leftFinalY, rightFinalY);

    // --- 일금 OOO원정 ---
    ensureSpace(S(30));
    text(numberToKoreanWon(grandTotal), MARGIN_X, S(11), { bold: true });
    y -= S(16);
    text(`(￦ ${fmtMoney(grandTotal)}원정), VAT별도`, MARGIN_X, S(9), { bold: true });
    y -= ROW_H; // 밑줄(품명·호기·규격... 표 헤더)까지 공백 한 줄만
    ...
    function drawTableHeader() {
      ...
      COLS.forEach((col) => {
        const w = fontBold.widthOfTextAtSize(col.label, S(9));
        const cellX = col.align === "right" ? x + col.width - 6 - w : col.align === "center" ? x + col.width / 2 - w / 2 : x + 4;
        text(col.label, cellX, S(9), { bold: true });
        x += col.width;
      });
      y -= HEADER_H / 2 - headerAscent / 2;
      line(MARGIN_X, MARGIN_X + CONTENT_W, 1);
      y -= S(10); // 4pt였을 땐 다음 줄(예: "1.자재비") 글자 위쪽이 이 선과 겹쳐 취소선처럼 보였음
    }
    drawTableHeader();
    ...
    function drawRow(cells, opts = {}) {
      ...
      COLS.forEach((col) => {
        ...
        const str = truncateToWidth(opts.bold ? fontBold : font, S(9), raw, col.width - 8);
        const w = (opts.bold ? fontBold : font).widthOfTextAtSize(str, S(9));
        const cellX = col.align === "right" ? x + col.width - 6 - w : col.align === "center" ? x + col.width / 2 - w / 2 : x + 4;
        text(str, cellX, S(9), { bold: opts.bold, color: opts.color });
        x += col.width;
      });
      y -= ROW_H;
    }
    ...
    function drawSectionLabel(label) {
      ensureSpace(ROW_H);
      text(label, MARGIN_X + 4, S(9), { bold: true, color: ACCENT });
      y -= ROW_H;
    }
    ...
    ["자재비", "인건비"].forEach((category, idx) => {
      ...
      // 품목 수와 무관하게 카테고리 아래에 한 줄만 여백을 둔다.
      ensureSpace(CATEGORY_PAD);
      y -= CATEGORY_PAD;
    });
    ...
    // --- 특이사항 ---
    ensureSpace(S(20));
    text("특이사항", MARGIN_X, S(9), { bold: true });
    y -= S(14);
    ...
    NOTES.forEach((note, idx) => {
      ...
      const lines = wrapText(wrapFont, S(8), note.text, noteMaxW);
      lines.forEach((lineStr, li) => {
        ensureSpace(S(14));
        if (li === 0) text(`${idx + 1}) `, MARGIN_X, S(8), opts);
        text(lineStr, noteBodyX, S(8), opts);
        y -= S(13);
      });
    });
