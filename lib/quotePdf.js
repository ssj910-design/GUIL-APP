...
import { COMPANY, BRAND } from "./company.js";
import { quoteDisplayTotal } from "./utils.js";
...
// (x,yTop)은 박스의 좌상단 모서리 — 이 파일의 "y가 아래로 갈수록 작아지는" 커서와
// 같은 좌표계를 쓴다. pdf-lib의 drawRectangle은 y가 좌하단 기준이라 yTop - h로 보정한다.
function drawSquareRect(page, { x, yTop, w, h, color, borderColor, borderWidth }) {
  page.drawRectangle({ x, y: yTop - h, width: w, height: h, color, borderColor, borderWidth });
}
...
export async function buildQuotePdfBytes(quote) {
  ...
  let sealImage = null;
  const sealPath = path.join(process.cwd(), BRAND.assets.seal);
  ...
  const preDiscountSubtotal = totalItems + Number(quote.transportCost || 0) + Number(quote.safetyCost || 0) + Number(quote.profit || 0);
  const subtotal = preDiscountSubtotal - discountAmount;
  const grandTotal = Math.floor(subtotal / 1000) * 1000;
  // 할인율은 저장된 값이 아니라 할인 적용 전 소계 대비로 매번 계산한다 — QuoteItemsModal
  // 화면과 같은 기준값(preDiscountSubtotal)이라 항상 서로 일치한다.
  const discountPercent = preDiscountSubtotal > 0 ? Math.round((discountAmount / preDiscountSubtotal) * 1000) / 10 : 0;
  // 관공서는 부가세포함으로 발행해야 해서(QuoteItemsModal의 VAT포함 체크), 체크됐으면
  // 표시 금액에 10%를 얹는다 — grandTotal(저장·재발송 기준값) 자체는 그대로 둔다.
  const vatIncluded = !!quote.vatIncluded;
  ...
  function renderLayout() {
    ...
    text(BRAND.name, MARGIN_X + iconW + 9, headerTopY - headerRowH / 2 - nameAscent / 2, nameSize, { bold: true });
    ...
    // 우: 공급자 정보 박스 (둥근 테두리)
    drawSquareRect(page, { x: issuerX, yTop: blockTopY, w: ISSUER_W, h: ISSUER_H, borderColor: LINE, borderWidth: 0.75 });
    let iy = blockTopY - 12;
    const coSize = 10.125;
    const coAscent = fontBold.heightAtSize(coSize, { descender: false });
    iy -= coAscent;
    const coText = BRAND.legal;
    ...
    ensureSpace(50);
    const barH = 42;
    drawSquareRect(page, { x: MARGIN_X, yTop: y, w: CONTENT_W, h: barH, color: INK });
    const wordsSize = 11.25, figureSize = 14.25, vatSize = 8.25;
    const wordsAscent = fontBold.heightAtSize(wordsSize, { descender: false });
    text(numberToKoreanWon(displayTotal), MARGIN_X + 15, y - barH / 2 - wordsAscent / 2, wordsSize, { bold: true, color: WHITE });
    const vatText = vatLabel;
    const vatW = font.widthOfTextAtSize(vatText, vatSize);
    const figureText = `₩${fmtMoney(displayTotal)}`;
    ...
    function drawRow(cells, opts = {}) {
      ...
      if (!opts.noBorder) line(MARGIN_X, MARGIN_X + CONTENT_W, y - ROW_H, 0.75, opts.borderColor ?? (opts.tint ? AMBER_TINT_BORDER : LINE));
      y -= ROW_H;
    }
    ...
    drawRow({ name: "3. 운반비", spec: "운반비 및 폐자재수거", amount: fmtMoney(quote.transportCost) }, { tint: true, borderColor: LINE });
    ...
    // VAT포함 체크 시 부가세도 항목 중 하나로 눈에 보이게 줄을 그린다(관공서 견적용) —
    // 이후 할인 행 번호는 부가세 행이 있으면 한 칸 밀린다.
    let nextRowNo = 6;
    if (vatIncluded) {
      drawRow({ name: `${nextRowNo}. 부가세`, spec: "공급가액의 10%", amount: fmtMoney(displayTotal - grandTotal) }, { tint: true });
      nextRowNo += 1;
    }
    // 할인은 입력했을 때(0보다 클 때)만 줄 자체를 그린다 — 나머지 고정행(운반비 등)과 달리
    // 할인 없는 견적서엔 아예 안 보여야 한다는 요청.
    if (discountAmount > 0) {
      drawRow({ name: `${nextRowNo}. 할인`, spec: `할인율 ${discountPercent}%`, amount: `-${fmtMoney(discountAmount)}` }, { tint: true });
    }

    y -= 10; // padding-top:10px
    drawRow({ name: `합계 (${vatLabel})`, spec: "천단위 절사", amount: fmtMoney(displayTotal) }, { bold: true, color: RED, noBorder: true });
    ...
    drawSquareRect(page, { x: MARGIN_X, yTop: y, w: CONTENT_W, h: noteBoxH, color: AMBER_TINT, borderColor: AMBER_TINT_BORDER, borderWidth: 0.75 });
    let ny = y - 9;
    ...
    line(MARGIN_X, MARGIN_X + CONTENT_W, y, 0.75, LINE);
    y -= 7.5;
    const footText = `본 견적서는 유효기간 내에서만 유효합니다 · ${BRAND.name}`;
