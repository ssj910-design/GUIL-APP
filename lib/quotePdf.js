...
// 원본 엑셀 양식처럼 1.자재비/2.인건비 아래에 여유 공간을 넉넉히 둔다(실제 입력된
// 품목 수와 무관하게 항상 이만큼 더해서, 품목이 적어도 서식처럼 보이게 한다).
const CATEGORY_PAD_ROWS = { 자재비: 5, 인건비: 4 };
...
export async function buildQuotePdfBytes(quote) {
  ...
  ["자재비", "인건비"].forEach((category, idx) => {
    ...
    ensureSpace(ROW_H * CATEGORY_PAD_ROWS[category]);
    y -= ROW_H * CATEGORY_PAD_ROWS[category];
  });
