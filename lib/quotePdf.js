...
export async function buildQuotePdfBytes(quote) {
  ...
  if (logoPage) {
    ...
    text("GUIL ELEVATOR", MARGIN_X, 20, { bold: true, color: ACCENT });
  }
  text("견   적   서", MARGIN_X + CONTENT_W - 130, 24, { bold: true, color: ACCENT });
  y -= 44;
  ...
  // --- 일금 OOO원정 ---
  ensureSpace(30);
  // 자재비/인건비/운반비/안전관리비 모두 items 배열의 항목이라 이 한 합계에 다 포함된다
  // (이윤만 품목화하지 않고 별도 입력값으로 남는다).
  const totalItems = (quote.items ?? []).reduce((s, it) => s + Number(it.qty || 0) * Number(it.unitPrice || 0), 0);
  const subtotal = totalItems + Number(quote.profit || 0);
  ...
  ["자재비", "인건비", "운반비", "안전관리비"].forEach((category, idx) => {
  ...
  page.drawRectangle({ x: MARGIN_X, y: y - ROW_H * 2 + 4, width: CONTENT_W, height: ROW_H * 2, color: TOTAL_BG });
