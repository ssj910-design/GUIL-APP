...
export async function buildQuotePdfBytes(quote) {
  ...
  const subtotal = totalItems + Number(quote.transportCost || 0) + Number(quote.safetyCost || 0) + Number(quote.profit || 0) - discountAmount;
  ...
  function renderLayout() {
    ...
    if (discountAmount > 0) {
      drawRow({ name: "6. 할인", amount: `-${fmtMoney(discountAmount)}` }, { tint: true });
    }

    y -= 10; // padding-top:10px
    drawRow({ name: discountAmount > 0 ? "소계 (1+2+3+4+5+6)" : "소계 (1+2+3+4+5)", amount: fmtMoney(subtotal) }, { bold: true, noBorder: true });
