...
// A4 한 장에 담기는 분량이면 아래 5곳(섹션 사이 여백)에 남는 공간을 골고루 더해
// 페이지 하단에 여백이 뜨지 않게 한다. 품목이 많아 2페이지 이상으로 넘어가면(측정
// 단계에서 페이지가 1장을 넘으면) 늘리지 않고 기본 간격 그대로 둔다.
const STRETCH_POINTS = 5;
...
export async function buildQuotePdfBytes(quote) {
  ...
  // extraGap: 섹션 사이 여백에 더할 여유분(A4 한 장 채우기용). 실제로 그리기 전에
  // 0으로 한번 그려보고 남는 공간을 계산해서 다시 그린다(아래 renderLayout 참고).
  function renderLayout(extraGap) {
    ...
    function drawTableHeader() {
      ...
      let x = MARGIN_X;
      // 라벨을 박스 아래쪽에 붙여서 배치(위쪽 여유를 크게, 아래쪽 여유는 작게)한다.
      y -= 15;
    ...
    line(MARGIN_X, MARGIN_X + CONTENT_W, 1);
    y -= 10 + extraGap;
    // 소계/합계 두 줄을 박스 아래쪽에 붙여서 배치(위 여유 크게, 아래 여유 작게).
    page.drawRectangle({ x: MARGIN_X, y: y - 23, width: CONTENT_W, height: 40, color: TOTAL_BG });
  ...
  if (dry.pageCount === 1) {
    ...
    if (leftover > 0) {
      let candidate = leftover / STRETCH_POINTS;
      // 그래도 넘치면(드문 경우) 실제로 다시 그려서 확인하고 반으로 줄여 재시도한다.
      for (let i = 0; i < 6; i++) {
        const probe = renderLayout(candidate);
