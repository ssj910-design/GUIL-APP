...
// JPEG의 EXIF Orientation 태그(1~8)만 읽는다 — 픽셀을 디코딩하지 않고 헤더 앞부분만
// 파싱해서 밀리초 단위로 가볍다. 못 찾으면 1(회전 없음)로 본다.
async function readExifOrientation(file) {
  try {
    const buf = await file.slice(0, 128 * 1024).arrayBuffer();
    const view = new DataView(buf);
    if (view.getUint16(0) !== 0xffd8) return 1; // JPEG 아님
    let offset = 2;
    while (offset < view.byteLength - 4) {
      const marker = view.getUint16(offset);
      if (marker === 0xffe1) {
        const tiffOffset = offset + 10; // marker(2)+length(2)+"Exif\0\0"(6)
...
// 막을 이유가 아니다.
//
// createImageBitmap을 imageOrientation 옵션 없이 쓴다 — "from-image"(EXIF 반영해서
// 디코딩)가 일부 안드로이드 웹뷰에서 소프트웨어 회전 경로를 타면서 몇 배씩(3MB 사진이
// 13초+) 느려지는 게 실측으로 확인됐다. 대신 EXIF 태그를 위에서 직접 가볍게 읽어서,
// 회전은 캔버스에 그릴 때 직접 돌린다 — 결과(세로사진이 눕지 않는 것)는 동일하고
// 느린 경로만 피한다. 플립(orientation 2/4/5/7)은 폰카메라가 사실상 만들지 않는
// 경우라 다루지 않는다 — 그대로 둔다.
...
async function compressImage(file, steps = []) {
  try {
    ...
    steps.push([`exif읽기(값=${orientation})`, Date.now() - t]); t = Date.now();
    const rotated90 = orientation === 6 || orientation === 8;
    // imageOrientation 옵션을 아예 안 넘기면 이 웹뷰가 기본값을 "EXIF 반영해서 자동회전"으로
    // 취급하는 것으로 보인다(그래서 아래 수동회전이랑 겹쳐 이중회전 났었다) — "none"을
    // 명시해서 반드시 원본 그대로(회전 전) 받는다.
    const bitmap = await createImageBitmap(file, { imageOrientation: "none" });
    steps.push([`디코딩(원본 ${bitmap.width}x${bitmap.height})`, Date.now() - t]); t = Date.now();
    const scale = Math.min(1, MAX_DIMENSION / Math.max(bitmap.width, bitmap.height));
    const drawW = Math.round(bitmap.width * scale);
    const drawH = Math.round(bitmap.height * scale);
    const canvas = document.createElement("canvas");
    canvas.width = rotated90 ? drawH : drawW;
    canvas.height = rotated90 ? drawW : drawH;
    const ctx = canvas.getContext("2d");
    if (orientation === 3) ctx.transform(-1, 0, 0, -1, drawW, drawH);
    else if (orientation === 6) ctx.transform(0, 1, -1, 0, drawH, 0);
    else if (orientation === 8) ctx.transform(0, -1, 1, 0, 0, drawW);
    ctx.drawImage(bitmap, 0, 0, drawW, drawH);
    bitmap.close?.();
    steps.push(["캔버스그리기", Date.now() - t]); t = Date.now();
    const blob = await new Promise((resolve) => canvas.toBlob(resolve, "image/jpeg", JPEG_QUALITY));
    steps.push(["JPEG인코딩", Date.now() - t]);
