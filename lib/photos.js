...
export async function downloadPhoto(url, filename) {
  if (Capacitor.isNativePlatform()) {
    // 앱(Android WebView)은 blob: URL 다운로드를 받아주는 다운로드 매니저가 없어
    // a.click()이 조용히 아무 일도 안 한다 — 시스템 브라우저로 넘겨서 그쪽이 받게 한다.
...
export async function downloadPhotosAsZip(urls, zipName, baseName) {
  if (Capacitor.isNativePlatform()) {
    // zip은 앱 안에서 만든 메모리상 blob이라 시스템 브라우저로 넘길 URL 자체가 없다 —
    // 파일시스템 저장 플러그인 없이는 지원 불가. 조용히 실패하는 대신 이유를 알린다.
    throw new Error("앱에서는 전체 다운로드가 지원되지 않습니다. 사진을 한 장씩 다운로드해주세요.");
  }
  const zip = new JSZip();
