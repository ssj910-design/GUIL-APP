...
export async function uploadPhoto(file, folder) {
  ...
  if (isVideo) {
    // 프레임 캡처(seek)에 큰 파일일수록 몇 초씩 걸릴 수 있어, 이걸 기다리면 사용자는 영상
    // 업로드 자체가 다 끝난 뒤에도 "업로드 중" 상태를 그만큼 더 오래 보게 된다. 영상 업로드
    // 결과(URL)는 바로 돌려주고, 포스터는 백그라운드에서 이어서 올린다 — 실패해도(코덱
    // 문제 등) 조용히 넘어간다. 포스터는 부가 기능일 뿐 영상 업로드 자체를 막지 않는다.
    generateVideoPoster(upload)
      .then((poster) => poster && supabase.storage.from(PHOTO_BUCKET).upload(`${path}.poster.jpg`, poster))
      .catch(() => {});
  }
