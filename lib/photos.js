...
// 기기·브라우저가 실제로 디코딩한 프레임을 그대로 이미지로 남긴다. 실패하면(코덱 문제
// 등) null을 돌려주고, 호출부는 영상 업로드 자체를 막지 않는다 — 포스터는 있으면 좋은 부가 기능.
function generateVideoPoster(file) {
  return new Promise((resolve) => {
    let settled = false;
    const finish = (result) => { if (!settled) { settled = true; URL.revokeObjectURL(url); resolve(result); } };
