...
//
// 목표 크기를 먼저 알아내서 createImageBitmap에 처음부터 그 크기로 디코딩하라고
// 넘긴다(resizeWidth/resizeHeight) — 풀해상도로 다 디코딩한 다음 캔버스에서 축소하는
// 것보다 훨씬 빠르다(JPEG는 축소 디코딩이 원리상 훨씬 싸다). 최종 해상도·화질은 이전과
// 동일 — 화질을 낮추는 게 아니라 같은 결과를 더 적게 계산해서 만드는 것뿐.
// <img> 태그로 원본 크기만 먼저 읽는 이유: 이것도 픽셀 전체를 그릴 필요가 없어
// createImageBitmap 풀디코딩보다 가볍다.
