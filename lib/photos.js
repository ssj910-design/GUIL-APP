...
// 목록 썸네일용 작은 이미지 URL — Supabase Storage 공개 이미지면 서버 리사이즈 경로(render/image)로
// 바꿔 준다. 청구내역 썸네일(48px)이 원본(보통 수백 KB)을 통째로 받던 게 느린 원인이었다
// (실측: 원본 242KB → 96px 2.7KB). 확대 보기는 계속 원본 URL을 쓴다. 영상·다른 호스트·이미지
// 아닌 파일은 그대로 돌려준다.
export function thumbUrl(url, size = 96) {
  if (!url || !url.includes("/storage/v1/object/public/") || !/\.(jpe?g|png|webp)(\?|$)/i.test(url)) return url;
  const [base] = url.split("?");
  return `${base.replace("/storage/v1/object/public/", "/storage/v1/render/image/public/")}?width=${size}&height=${size}&resize=cover&quality=60`;
}
