...
export function mergeAssignedEngineers(mappedSites, siteAssignmentRows, profileRows) {
  ...
  for (const arr of bySite.values()) arr.sort((x, y) => Number(y.isLead) - Number(x.isLead) || x.name.localeCompare(y.name, "ko"));

  return mappedSites.map((s) => {
    const original = s.assignedEngineer; // mapSite가 채운 옛 컬럼 값 — site_assignments가 비어있을 때만 fallback으로 쓴다.
    let names = (bySite.get(s.id) ?? []).map((x) => x.name);
    // site_assignments에 이 현장 행이 하나도 없을 때만 옛 컬럼으로 보완한다(백필 시차·일시적 조회
    // 실패 대비). site_assignments에 행이 하나라도 있으면 그게 항상 이긴다 — 드리프트 재도입 방지.
    if (names.length === 0 && original) names = [original];
...
export function mapFailure(row) {
  return {
    ...
    inProgressAt: row.in_progress_at, // 출동 시각(timestamptz) — 처리결과 5시간 미입력 크론 판정용. 컬럼 없으면 undefined
    ...
    cancelledAt: row.cancelled_at,     // 컬럼 없으면(마이그레이션 140 전) undefined → 취소 버튼 숨김
...
export function mapTodo(row) {
  return {
    ...
    siteName: row.site_name,
    elevatorNo: row.elevator_no,
    elevatorNos: row.elevator_nos ?? null,
    part: row.part,
    assignee: row.assignee,
...
export function mapQuoteRequest(row) {
  return {
    ...
    siteName: row.site_name,
    elevatorNo: row.elevator_no,
    elevatorNos: row.elevator_nos ?? null, // 한 요청에 여러 호기가 걸려있으면 전체 목록(없으면 null, 컬럼 마이그레이션 전이거나 단일 호기)
    constructionType: row.construction_type,
    contactPhone: row.contact_phone,
    contactEmail: row.contact_email, // 선택 입력 — 마이그레이션 145 전이면 undefined
    contactFax: row.contact_fax,
    note: row.note,
    photoCount: row.photo_count,
    photoUrls: row.photo_urls ?? [],
    engineer: row.engineer,
...
export function mapBilling(row) {
  return {
    ...
    receivedPayments: row.received_payments ?? null, // 분할납부 목록 [{date, amount}] — 마이그레이션 138
...
export function mapDutySchedule(row) {
  return {
    ...
    kind: row.kind, // 당직 | 숙직
    profileId: row.profile_id,        // 실제 근무자 — 교환·대신서기·수동변경이 반영된 값
    autoProfileId: row.auto_profile_id, // 순번이 뽑은 사람(원래 순번) — 다음 달 이어받기 기준. 마이그레이션 144 전이면 undefined
  };
}
