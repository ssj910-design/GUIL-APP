...
export async function recordQuoteSupplyStockOut(supabase, input) {
  await insertMovements(supabase, quoteStockRows(input, "out"));
}

// 자재지급완료 뒤 견적을 취소하면서 지급 자재가 창고로 돌아왔을 때 — 지급 때 뺀 수량을
// 그대로 되돌린다(lib/quoteReject.js).
export async function recordQuoteRejectStockIn(supabase, input) {
  await insertMovements(supabase, quoteStockRows(input, "in"));
}

export function quoteStockRows({ quoteItems, quoteId, siteName, createdBy }, type) {
  const sign = type === "out" ? -1 : 1;
  return (quoteItems ?? []).filter((it) => it.partId).map((it) => ({
    product_id: it.partId,
    type,
    qty_delta: sign * Number(it.qtyTaken ?? it.qty),
    note: type === "out" ? `견적 ${quoteId} 지급` : `견적 ${quoteId} 취소 반납`,
    site_text: siteName,
    created_by: createdBy ?? null,
  }));
}

async function insertMovements(supabase, rows) {
