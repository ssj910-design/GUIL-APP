...
// 근무표 생성(근무제 선택) — 관리자웹콘솔(DutyAdmin.jsx)과 모바일앱(WorkCalendarSheet) 양쪽에서
// 똑같이 쓰는 위젯. DutyRoster의 belowCalendar 슬롯에 꽂아 쓴다.
// 읽기(engineers·profiles)는 AuthContext에서, 쓰기는 onSchedulesChange/onEngineersChange로 위임해
// 관리자웹의 data/setData든 모바일의 개별 state든 그대로 붙는다.
...
export function DutyGenerateWidget({ schedules, onSchedulesChange, onEngineersChange }) {
  const { engineers, profiles } = useContext(AuthContext);
  ...
  async function generate(ym, mode = "주5일") {
    ...
    onSchedulesChange((p) => [...p.filter((x) => !mapped.some((n) => n.id === x.id)), ...mapped]
  ...
  async function retract(ym) {
    ...
    onSchedulesChange((prev) => prev.filter((s) => !(s.dutyDate >= from && s.dutyDate <= to)));
    setManualOverrides((prev) => {
      const next = { ...prev };
  ...
  async function saveOrder(p, value) {
    ...
    onEngineersChange((prev) => prev.map((x) => (x.id === p.id ? { ...x, duty_order } : x)));
  }
  ...
  async function toggleMode(p, mode, on) {
    ...
    onEngineersChange((prev) => prev.map((x) => (x.id === p.id ? { ...x, duty_modes } : x)));
  }
  ...
  async function simulate(ym, mode, roster) {
    ...
    const nameOfAny = (pid) => profiles.find((p) => p.id === pid)?.name ?? "";
    if (!roster.length) {
    ...
    <div className="bg-white border border-slate-200 rounded-xl p-4">
      ...
      <div className="mb-3">
          ...
          {calLoading ? (
            ...
            <div className="grid grid-cols-7">
              {Array.from({ length: genMonthStartDow }, (_, i) => (
                <div key={`pad${i}`} className="border-b border-r border-slate-100 min-h-[56px]" />
              ))}
              {Array.from({ length: daysInGenMonth }, (_, i) => i + 1).map((d) => {
                ...
                return (
                  <div key={d} className={`border-b border-r border-slate-100 min-h-[56px] p-1 ${hasNew ? "bg-blue-50/40" : ""}`}>
        ...
        <div className="flex items-center gap-2.5 flex-wrap text-[11px] text-slate-500 mt-2">
          ...
          <span className="flex items-center gap-1 font-semibold"><span className="w-2.5 h-2.5 rounded bg-blue-100" /> 자동</span>
          <span className="flex items-center gap-1 font-semibold"><span className="w-2.5 h-2.5 rounded bg-amber-100" /> 수동</span>
        </div>
      </div>
      ...
      <div className="flex gap-2">
        ...
        <button
          ...
          disabled={retracting}
          className="text-sm font-bold text-red-600 bg-red-50 border border-red-100 rounded-xl px-4 disabled:opacity-50 shrink-0"
        >
