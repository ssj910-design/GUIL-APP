...
export default function ErrorCodesAdmin({ data, setData }) {
  ...
  async function importErrorCodesFile(file) {
    try {
      ...
      // (Postgres 제약) — 임의로 하나를 골라 덮어쓰지 않고, 어느 게 맞는지 사람이 확인하게 막는다.
      const seenCount = new Map();
      for (const r of parsed) {
        const k = `${r.model}::${r.code}`;
        seenCount.set(k, (seenCount.get(k) ?? 0) + 1);
      }
      const dupKeys = [...seenCount.entries()].filter(([, n]) => n > 1).map(([k]) => k.replace("::", " · "));
      if (dupKeys.length > 0) {
        alert(`파일 안에 같은 (기종, 코드) 조합이 중복돼있어 업로드할 수 없습니다.\n확인 후 하나만 남기고 다시 올려주세요.\n\n중복: ${dupKeys.join(", ")}`);
        return;
      }
